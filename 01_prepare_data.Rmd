...
sample_metadata <- read_tsv("data/DMB0029_metadata.tsv.gz") %>%
    rename(sample=1)
```
...
read_counts <- read_tsv("data/DMB0029_counts.tsv.gz") %>%
    rename(genome=1) %>% 
...
genome_metadata <- read_tsv("data/DMB0029_mag_info.tsv.gz") %>%
...
genome_coverage <- read_tsv("data/DMB0029_coverage.tsv.gz") %>%
    rename(genome=1) %>% 
    select(one_of(c("genome",sample_metadata$sample)))%>% 
...
genome_tree <- read_tree("data/DMB0029.tree.gz")
